...
while ( ($data{def}, $data{num})=$data{sth}->fetchrow_array() )
	...
	if ( $data{rows} > 0 )
		...
		$dbh->do("insert into abc_provider_tst (def,start_num,end_num,quantity,provider_nm,region_nm,tariff_zone) values (\'$data{def_r}\',\'$data{start_num}\',\'$data{end_num}\',\'$data{quantity}\',\'$data{provider_nm}\',\'$data{region_nm}\',\'$data{tariff_zone}\')");
